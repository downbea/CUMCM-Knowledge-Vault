---
id: model-correlation-analysis
title: 相关性分析
english_name: Correlation Analysis
...
tags: [模型, core, 统计分析]
use_cases: [特征筛选、关系探索]
code_example: "[[40-Code-Examples/correlation-analysis/README|Python 示例]]"
...
特征筛选、关系探索。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

相关不等于因果；离群点和样本量会影响结论。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

Pearson、Spearman、Kendall按数据条件选择。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：相关系数、置信区间、显著性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/correlation-analysis/README|相关性分析可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
