...
id: model-sir-ode
title: SIR传染病模型
english_name: SIR Epidemic Model
aliases: [SIR Epidemic Model]
tier: high-frequency
category: 微分方程
status: seeded
decision: pending
...
tags: [模型, high-frequency, 微分方程]
use_cases: [传播、扩散、舆情类比]
code_example: "[[40-Code-Examples/sir-ode/README|Python 示例]]"
...
用易感、感染和康复人群流动描述传播动力学。

## 适用问题

传播、扩散、舆情类比。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

封闭均匀混合等假设常较强。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

建立ODE并数值积分。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：拟合误差、R0、参数敏感性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/sir-ode/README|SIR传染病模型可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
...
- SciPy integrate official documentation.
