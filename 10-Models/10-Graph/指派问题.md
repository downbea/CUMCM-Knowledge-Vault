---
id: model-assignment-problem
title: 指派问题
english_name: Assignment Problem
aliases: [Assignment Problem]
tier: high-frequency
category: 优化与匹配
status: seeded
decision: pending
...
tags: [模型, high-frequency, 优化与匹配]
use_cases: [人员任务分配、匹配]
code_example: "[[40-Code-Examples/assignment-problem/README|Python 示例]]"
...
将任务一一分配给资源以最小化成本或最大化收益。

## 适用问题

人员任务分配、匹配。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

经典形式要求一对一和可加成本。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

匈牙利算法或线性规划。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：总成本、匹配可行性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/assignment-problem/README|指派问题可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
