---
id: model-maximum-flow
title: 最大流
english_name: Maximum Flow
...
use_cases: [运输能力、供应网络]
code_example: "[[40-Code-Examples/maximum-flow/README|Python 示例]]"
...
运输能力、供应网络。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

需要合理容量和流守恒定义。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

增广路径或预流推进。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：最大流、最小割、瓶颈边。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/maximum-flow/README|最大流可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
