---
id: model-robust-regression
title: 稳健回归
english_name: Robust Regression
aliases: [Robust Regression]
tier: high-frequency
category: 回归分析
status: seeded
decision: pending
...
tags: [模型, high-frequency, 回归分析]
use_cases: [异常样本较多的关系建模]
code_example: "[[40-Code-Examples/robust-regression/README|Python 示例]]"
...
在离群点或重尾噪声下获得更稳定的回归估计。

## 适用问题

异常样本较多的关系建模。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

仍需检查结构性错设和高杠杆点。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

Huber或RANSAC降低异常残差的影响。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：MAE、RMSE、系数稳定性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/robust-regression/README|稳健回归可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
