---
id: model-linear-regression
title: 多元线性回归
english_name: Multiple Linear Regression
aliases: [Multiple Linear Regression]
tier: core
category: 回归分析
status: seeded
decision: pending
...
tags: [模型, core, 回归分析]
use_cases: [影响因素、解释与预测]
code_example: "[[40-Code-Examples/linear-regression/README|Python 示例]]"
...
影响因素、解释与预测。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

线性、独立、同方差、残差条件和共线性需检查。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

最小二乘估计配合残差诊断与交叉验证。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：R²、调整R²、RMSE、VIF。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/linear-regression/README|多元线性回归可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
