...
tags: [模型, core, 优化]
use_cases: [资源分配、配比、运输]
code_example: "[[40-Code-Examples/linear-programming/README|Python 示例]]"
...
资源分配、配比、运输。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

变量、目标和约束必须可线性表达。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

单纯形或内点法求解。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：目标值、约束松弛、影子价格。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/linear-programming/README|线性规划可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
