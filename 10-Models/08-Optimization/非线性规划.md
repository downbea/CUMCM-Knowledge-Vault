---
id: model-nonlinear-programming
title: 非线性规划
english_name: Nonlinear Programming
aliases: [Nonlinear Programming]
tier: high-frequency
category: 优化
status: seeded
decision: pending
...
use_cases: [参数优化、工程设计]
code_example: "[[40-Code-Examples/nonlinear-programming/README|Python 示例]]"
...
参数优化、工程设计。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

局部最优、初值和可微性影响求解。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

梯度或无梯度数值优化。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：目标值、约束违反、初值敏感性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/nonlinear-programming/README|非线性规划可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
