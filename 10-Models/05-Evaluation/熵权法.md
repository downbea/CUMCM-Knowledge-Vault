---
id: model-entropy-weight
title: 熵权法
english_name: Entropy Weight Method
aliases: [Entropy Weight Method]
tier: high-frequency
category: 综合评价
status: seeded
decision: pending
...
use_cases: [多指标客观赋权]
code_example: "[[40-Code-Examples/entropy-weight/README|Python 示例]]"
...
多指标客观赋权。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

离散度大不必然代表决策重要性。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

标准化后用信息熵衡量指标区分度。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：权重稳定性、排名敏感性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/entropy-weight/README|熵权法可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
