---
id: model-grey-relational-analysis
title: 灰色关联分析
english_name: Grey Relational Analysis
aliases: [Grey Relational Analysis]
tier: high-frequency
category: 综合评价
status: seeded
decision: pending
...
use_cases: [小样本、信息不完全评价]
code_example: "[[40-Code-Examples/grey-relational-analysis/README|Python 示例]]"
...
小样本、信息不完全评价。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

分辨系数和无量纲化方式会影响结果。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

比较序列与参考序列的关联系数和关联度。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：关联度、排序敏感性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/grey-relational-analysis/README|灰色关联分析可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
