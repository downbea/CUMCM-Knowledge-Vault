...
id: model-ahp
title: 层次分析法
english_name: Analytic Hierarchy Process
aliases: [Analytic Hierarchy Process]
tier: high-frequency
category: 综合评价
status: seeded
decision: pending
...
use_cases: [专家判断、多准则决策]
code_example: "[[40-Code-Examples/ahp/README|Python 示例]]"
...
将复杂决策分层并通过成对比较确定主观权重。

## 适用问题

专家判断、多准则决策。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

判断矩阵需要一致性且专家意见可解释。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

特征向量法计算权重并进行一致性检验。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：CI、CR、权重敏感性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/ahp/README|层次分析法可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
