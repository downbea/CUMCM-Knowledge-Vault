---
id: model-combination-weight
title: 组合赋权
english_name: Combined Weighting
aliases: [Combined Weighting]
tier: high-frequency
category: 综合评价
status: seeded
decision: pending
...
use_cases: [综合评价权重确定]
code_example: "[[40-Code-Examples/combination-weight/README|Python 示例]]"
...
综合评价权重确定。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

组合规则需解释且应做敏感性分析。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

线性组合、博弈组合或最小偏差方法。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：组合系数、排名稳定性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/combination-weight/README|组合赋权可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
