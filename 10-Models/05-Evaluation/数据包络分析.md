...
id: model-dea
title: 数据包络分析
english_name: Data Envelopment Analysis
aliases: [Data Envelopment Analysis]
tier: high-frequency
category: 综合评价
status: seeded
decision: pending
...
use_cases: [效率评价、资源配置]
code_example: "[[40-Code-Examples/dea/README|Python 示例]]"
...
用线性规划衡量多投入多产出决策单元的相对效率。

## 适用问题

效率评价、资源配置。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

样本数需相对充分且变量方向明确。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

CCR/BCC模型构造效率前沿。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：效率值、松弛量、规模报酬。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/dea/README|数据包络分析可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
