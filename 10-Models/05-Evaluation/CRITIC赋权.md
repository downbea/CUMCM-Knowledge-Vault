---
id: model-critic-weight
title: CRITIC赋权
english_name: CRITIC Weighting
aliases: [CRITIC Weighting]
tier: high-frequency
category: 综合评价
status: seeded
decision: pending
...
use_cases: [相关指标较多的客观赋权]
code_example: "[[40-Code-Examples/critic-weight/README|Python 示例]]"
...
相关指标较多的客观赋权。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

对相关系数和标准化方式敏感。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

标准差反映信息量，相关性反映冲突性。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：权重、相关结构、敏感性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/critic-weight/README|CRITIC赋权可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
