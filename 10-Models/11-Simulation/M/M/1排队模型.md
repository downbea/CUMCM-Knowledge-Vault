...
tags: [模型, high-frequency, 运筹与仿真]
use_cases: [服务系统容量与等待分析]
code_example: "[[40-Code-Examples/mm1-queue/README|Python 示例]]"
...
服务系统容量与等待分析。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

要求到达率小于服务率且分布假设合理。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

稳态概率与Little定律推导指标。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：利用率、等待时间、队长。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/mm1-queue/README|M/M/1排队模型可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
