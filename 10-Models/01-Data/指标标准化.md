...
title: 指标标准化
english_name: Normalization and Scaling
aliases: [Normalization and Scaling]
tier: core
category: 数据处理
status: seeded
decision: pending
...
tags: [模型, core, 数据处理]
use_cases: [综合评价、机器学习、距离模型]
code_example: "[[40-Code-Examples/normalization/README|Python 示例]]"
...
统一量纲并处理正向、负向、中间型和区间型指标。

## 适用问题

综合评价、机器学习、距离模型。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

需明确指标方向、单位和异常值。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

极差、Z-score、向量归一化及指标正向化。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：分布、尺度、排名稳定性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/normalization/README|指标标准化可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
