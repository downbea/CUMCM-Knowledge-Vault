---
id: model-outlier-detection
title: 异常值检测
english_name: Outlier Detection
...
tags: [模型, core, 数据处理]
use_cases: [数据审计、稳健建模]
code_example: "[[40-Code-Examples/outlier-detection/README|Python 示例]]"
...
数据审计、稳健建模。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

异常不等于错误，删除必须有业务或统计依据。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

IQR、Z-score和Isolation Forest提供不同假设下的异常度量。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：异常比例、处理前后统计、敏感性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/outlier-detection/README|异常值检测可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
