---
id: model-missing-value-imputation
title: 缺失值插补
english_name: Missing Value Imputation
aliases: [Missing Value Imputation]
tier: core
category: 数据处理
status: seeded
decision: pending
...
tags: [模型, core, 数据处理]
use_cases: [缺失值填补、数据预处理]
code_example: "[[40-Code-Examples/missing-value-imputation/README|Python 示例]]"
...
缺失值填补、数据预处理。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

缺失机制与字段含义需要先分析。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

均值/中位数、插值、KNN等方法应按变量类型和缺失机制选择。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：缺失率、分布变化、下游模型稳定性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/missing-value-imputation/README|缺失值插补可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
