---
id: model-kalman-filter
title: 卡尔曼滤波
english_name: Kalman Filter
...
tags: [模型, high-frequency, 状态估计]
use_cases: [跟踪、传感器融合、动态预测]
code_example: "[[40-Code-Examples/kalman-filter/README|Python 示例]]"
...
跟踪、传感器融合、动态预测。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

线性高斯模型是经典形式的关键条件。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

预测与校正递推最小化估计协方差。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：RMSE、创新序列、协方差。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/kalman-filter/README|卡尔曼滤波可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
