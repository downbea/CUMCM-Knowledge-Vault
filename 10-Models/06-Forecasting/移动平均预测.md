---
id: model-moving-average
title: 移动平均预测
english_name: Moving Average Forecast
aliases: [Moving Average Forecast]
tier: core
category: 时间序列
status: seeded
decision: pending
...
tags: [模型, core, 时间序列]
use_cases: [平稳或缓慢变化序列]
code_example: "[[40-Code-Examples/moving-average/README|Python 示例]]"
...
用近期观测的平均值平滑噪声并进行短期预测。

## 适用问题

平稳或缓慢变化序列。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

窗口大小决定平滑与滞后。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

滚动窗口平均作为基线预测。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：MAE、RMSE、滚动回测。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/moving-average/README|移动平均预测可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
