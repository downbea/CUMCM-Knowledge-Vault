...
use_cases: [线性、可平稳化时间序列]
code_example: "[[40-Code-Examples/arima/README|Python 示例]]"
...
通过差分和自回归移动平均结构预测单变量序列。

## 适用问题

线性、可平稳化时间序列。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

需检查平稳性、残差白噪声和阶数。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

差分后拟合ARMA并诊断残差。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：AIC、BIC、RMSE、Ljung-Box。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/arima/README|ARIMA模型可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
