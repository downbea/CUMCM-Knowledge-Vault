...
id: model-gm11
title: GM(1,1)灰色预测
english_name: Grey Model GM(1,1)
aliases: [Grey Model GM(1,1)]
tier: high-frequency
category: 时间序列
status: seeded
decision: pending
...
use_cases: [小样本、单调趋势序列]
code_example: "[[40-Code-Examples/gm11/README|Python 示例]]"
...
用累加生成弱化随机性并建立一阶灰色微分方程。

## 适用问题

小样本、单调趋势序列。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

级比与准指数规律不满足时不宜使用。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

AGO累加、参数最小二乘估计和还原预测。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：后验差比、小误差概率、相对误差。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/gm11/README|GM(1,1)灰色预测可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
