---
id: model-exponential-smoothing
title: 指数平滑
...
tags: [模型, core, 时间序列]
use_cases: [无明显复杂结构的短期预测]
code_example: "[[40-Code-Examples/exponential-smoothing/README|Python 示例]]"
...
无明显复杂结构的短期预测。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

平滑参数和趋势结构需通过验证选择。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

简单指数平滑递推更新水平项。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：MAE、RMSE、参数稳定性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/exponential-smoothing/README|指数平滑可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
