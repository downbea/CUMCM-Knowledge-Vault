---
id: model-holt-winters
title: Holt-Winters模型
english_name: Holt-Winters Exponential Smoothing
aliases: [Holt-Winters Exponential Smoothing]
tier: high-frequency
category: 时间序列
status: seeded
decision: pending
...
tags: [模型, high-frequency, 时间序列]
use_cases: [具有趋势与季节性的序列]
code_example: "[[40-Code-Examples/holt-winters/README|Python 示例]]"
...
具有趋势与季节性的序列。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

季节周期需可靠且历史长度足够。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

加法或乘法季节模型递推估计。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：RMSE、MAPE、滚动回测。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/holt-winters/README|Holt-Winters模型可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
