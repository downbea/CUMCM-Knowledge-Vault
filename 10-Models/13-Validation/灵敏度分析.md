---
id: model-sensitivity-analysis
title: 灵敏度分析
english_name: Sensitivity Analysis
...
tags: [模型, core, 模型检验]
use_cases: [所有重要模型的稳健性检验]
code_example: "[[40-Code-Examples/sensitivity-analysis/README|Python 示例]]"
...
所有重要模型的稳健性检验。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

扰动范围应有实际依据。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

单因素、局部或全局扰动比较输出。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：弹性、排序变化、稳定区间。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/sensitivity-analysis/README|灵敏度分析可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
