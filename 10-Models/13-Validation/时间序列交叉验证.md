---
id: model-time-series-cross-validation
title: 时间序列交叉验证
english_name: Time Series Cross Validation
aliases: [Time Series Cross Validation]
tier: core
category: 模型检验
status: seeded
decision: pending
...
tags: [模型, core, 模型检验]
use_cases: [时间序列模型比较]
code_example: "[[40-Code-Examples/time-series-cross-validation/README|Python 示例]]"
...
按时间顺序滚动评估预测模型避免未来信息泄漏。

## 适用问题

时间序列模型比较。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

不能随机打乱，窗口方案需匹配应用。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

扩展窗口或滚动窗口回测。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：各折误差、均值、波动。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/time-series-cross-validation/README|时间序列交叉验证可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
