...
id: model-pca
title: 主成分分析
english_name: Principal Component Analysis
...
tags: [模型, core, 降维与评价]
use_cases: [降维、综合评价、可视化]
code_example: "[[40-Code-Examples/pca/README|Python 示例]]"
...
降维、综合评价、可视化。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

变量应可比较且线性结构有意义。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

特征分解或SVD最大化投影方差。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：累计解释方差、载荷、重构误差。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/pca/README|主成分分析可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
