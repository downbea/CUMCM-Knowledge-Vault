---
id: model-hierarchical-clustering
title: 层次聚类
...
use_cases: [样本层次关系分析]
code_example: "[[40-Code-Examples/hierarchical-clustering/README|Python 示例]]"
...
样本层次关系分析。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

距离和链接方式影响结果，规模大时昂贵。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

凝聚式聚类构建树状图。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：树状图、轮廓系数、稳定性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/hierarchical-clustering/README|层次聚类可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
