---
id: model-random-forest-classification
title: 随机森林分类
english_name: Random Forest Classification
...
tags: [模型, high-frequency, 分类]
use_cases: [表格分类与变量重要性]
code_example: "[[40-Code-Examples/random-forest-classification/README|Python 示例]]"
...
表格分类与变量重要性。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

概率可能需校准且不擅长外推。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

多树投票或概率平均。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：F1、AUC、OOB、特征重要性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/random-forest-classification/README|随机森林分类可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
