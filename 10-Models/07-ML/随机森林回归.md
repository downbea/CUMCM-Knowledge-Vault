---
id: model-random-forest-regression
title: 随机森林回归
english_name: Random Forest Regression
aliases: [Random Forest Regression]
tier: high-frequency
category: 机器学习回归
status: seeded
decision: pending
...
use_cases: [表格数据非线性预测]
code_example: "[[40-Code-Examples/random-forest-regression/README|Python 示例]]"
...
表格数据非线性预测。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

外推能力有限，需防止数据泄漏。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

Bootstrap样本与随机特征建立树并平均。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：交叉验证RMSE、MAE、特征重要性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/random-forest-regression/README|随机森林回归可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
