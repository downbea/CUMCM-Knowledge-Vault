...
use_cases: [存在密度簇和异常点]
code_example: "[[40-Code-Examples/dbscan/README|Python 示例]]"
...
存在密度簇和异常点。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

不同密度数据参数难统一。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

基于邻域半径和最小样本形成密度可达簇。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：噪声率、轮廓系数、参数敏感性。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/dbscan/README|DBSCAN聚类可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
