---
id: model-gradient-boosting-regression
title: 梯度提升回归
english_name: Gradient Boosting Regression
aliases: [Gradient Boosting Regression]
tier: high-frequency
category: 机器学习回归
status: seeded
decision: pending
...
tags: [模型, high-frequency, 机器学习回归]
use_cases: [中小型表格数据预测]
code_example: "[[40-Code-Examples/gradient-boosting-regression/README|Python 示例]]"
...
中小型表格数据预测。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

对参数和噪声敏感，解释成本较高。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

按负梯度方向迭代添加弱学习器。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：交叉验证、学习曲线、SHAP。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/gradient-boosting-regression/README|梯度提升回归可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
