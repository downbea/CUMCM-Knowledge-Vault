...
use_cases: [中小样本高维分类]
code_example: "[[40-Code-Examples/svm/README|Python 示例]]"
...
中小样本高维分类。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

需标准化且参数选择影响较大。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

软间隔优化与核技巧。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：F1、AUC、交叉验证。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/svm/README|支持向量机可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
