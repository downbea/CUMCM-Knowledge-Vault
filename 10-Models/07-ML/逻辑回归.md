...
tags: [模型, core, 分类]
use_cases: [二分类、多分类基线]
code_example: "[[40-Code-Examples/logistic-regression/README|Python 示例]]"
...
二分类、多分类基线。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

线性决策边界、样本独立性和共线性需检查。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

最大似然估计类别对数几率。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：AUC、F1、校准、系数解释。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/logistic-regression/README|逻辑回归可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
