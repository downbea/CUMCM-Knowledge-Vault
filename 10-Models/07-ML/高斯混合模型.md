---
id: model-gaussian-mixture
title: 高斯混合模型
english_name: Gaussian Mixture Model
aliases: [Gaussian Mixture Model]
tier: high-frequency
category: 聚类
status: seeded
decision: pending
...
tags: [模型, high-frequency, 聚类]
use_cases: [椭球簇、软聚类]
code_example: "[[40-Code-Examples/gaussian-mixture/README|Python 示例]]"
...
椭球簇、软聚类。系统只能把本方法列为候选，不能仅凭关键词直接确定为主模型。

## 前置条件与数据要求

高斯假设和组件数需评估。使用前应检查字段含义、样本量、量纲、缺失值、异常值和数据泄漏风险。

## 数学原理

EM算法交替估计责任度与参数。正式论文需要结合当前题目的变量、约束与假设重新推导，不能只粘贴通用公式。
...
重点检查：BIC、AIC、对数似然。复杂或随机模型必须报告多次运行或交叉验证的平均结果和波动。
...
见 [[40-Code-Examples/gaussian-mixture/README|高斯混合模型可运行示例]]。示例用于验证基本机制，不代表已经复现任何具体论文或国赛题。
